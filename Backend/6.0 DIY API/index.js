...
app.get("/filter", (req, res) => {
  ...
  const filteredType = jokes.filter((joke) => joke.jokeType === type);
...
app.patch("/jokes/:id", (req, res) => {
  ...
  const existingJoke = jokes[existingIndex];
  const jokeUpdate = {
    id: id,
  ...
  jokes[existingIndex] = jokeUpdate;
  res.json(jokeUpdate);
})
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if(searchIndex > -1){
    ...
    res
      .status(404)
      .json({error: `joke with id: ${id} is not found. No jokes were deleted`});
  }
});
